...
export default function PosLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!["OWNER", "MANAGER", "STAFF"].includes(res.data.user.role)) {
        toast.error("This account isn't authorized for POS.");
        return;
      }
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="flex items-center gap-2.5 mb-8">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">POS Login</p>
          </div>
        </div>

        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
          <h1 className="text-xl font-bold text-slate-900 mb-1">POS Sign in</h1>
          <p className="text-sm text-slate-500 mb-6">Staff login for the point of sale</p>
